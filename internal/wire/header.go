...
// EncryptedHeader is the header of a QUIC packet.
...
func (h *EncryptedHeader) Write(b *bytes.Buffer, pers protocol.Perspective, ver protocol.VersionNumber) error {
	if h.IsLongHeader {
...
func (h *EncryptedHeader) writeLongHeader(b *bytes.Buffer, v protocol.VersionNumber) error {
...
func (h *EncryptedHeader) writeShortHeader(b *bytes.Buffer, v protocol.VersionNumber) error {
	typeByte := byte(0x30)
...
func (h *EncryptedHeader) GetLength(v protocol.VersionNumber) protocol.ByteCount {
	if h.IsLongHeader {
...
// Log logs the Header
func (h *EncryptedHeader) Log(logger utils.Logger) {
	if h.IsLongHeader {
		if h.Version == 0 {
