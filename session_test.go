...
var _ = Describe("Session", func() {
	...
	Context("frame handling", func() {
		...
		Context("handling ACK frames", func() {
			...
			It("tells the ReceivedPacketHandler to ignore low ranges", func() {
				...
				Expect(sess.handleAckFrame(ack, protocol.EncryptionInitial)).To(Succeed())
			})
		})
	...
	Context("receiving packets", func() {
		var hdr *wire.EncryptedHeader
		var unpacker *MockUnpacker

		BeforeEach(func() {
			unpacker = NewMockUnpacker(mockCtrl)
			sess.unpacker = unpacker
			hdr = &wire.EncryptedHeader{PacketNumberLen: protocol.PacketNumberLen4}
		})
		...
		Context("updating the remote address", func() {
			It("doesn't support connection migration", func() {
				...
				p := receivedPacket{
					remoteAddr: remoteIP,
					header:     &wire.EncryptedHeader{PacketNumber: 1337},
				}
	...
	Context("sending packets", func() {
		getPacket := func(pn protocol.PacketNumber) *packedPacket {
			...
			return &packedPacket{
				raw:    data,
				header: &wire.EncryptedHeader{PacketNumber: pn},
			}
		}
